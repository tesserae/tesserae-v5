        ...
        if parallel:
            with ThreadPool() as p:
                match_ents = p.starmap(score, [(unit_lists[0][i], [unit_lists[1][j] for j in matches[i].nonzero()[1]],
                                            frequencies, features, distance_metric,
                                            max_distance, min_score, match_set)
                                            for i in range(matches.shape[0])])
            match_ents = list(itertools.chain.from_iterable(match_ents))
        else:
            for i in range(matches.shape[0]):
...
def score(source, targets, frequencies, features, distance_metric, maximum_distance, min_score, match_set):
    matches = []
    source_features = list(itertools.chain.from_iterable(source['features'][0]['features']))
    ...
    source_indices = list(itertools.chain.from_iterable(source_indices))
    source_frequencies = np.array([frequencies[i] for i in source_features])

    for target in targets:
        target_features = list(itertools.chain.from_iterable(target['features'][0]['features']))
        ...
        target_indices = list(itertools.chain.from_iterable(target_indices))
